...
по состоянию на 2026-07-14, коммит `8b4d260`, ветка `claude/smarttracker-ant-plus-hrm-18d856`.
...
- `SensorsUiState` (data class) — `permissionsGranted`, `savedDevices: List<SavedHrmDevice>`, `activeAddress: String?`, `connectionState: HrmConnectionState`, `currentBpm: Int?` (только при CONNECTED), `isScanning`, `hasScanned`, `scanResults: List<HrmScanResult>` (дедуп по адресу, сортировка по rssi); computed `foundDevices` — `scanResults` без уже сохранённых (добавленный через «+» реактивно уезжает из «найденных»).
- `SensorsViewModel` (`@HiltViewModel`, инжектит `HrmManager` + `SettingsStorage`) — `init`: зеркалит `combine(settings, connectionState, currentSample, isScanning)` в UiState; отдельный коллектор накапливает `scanResults`.
  - `fun onPermissionsGranted()` — ставит флаг + `maybeAutoScan()`.
  - `fun onScanClick()` — ручной скан на тап (повторный тап во время скана игнорируется), `startScan(SCAN_TIMEOUT_MS=30с)`.
...
- `@Composable fun SensorsScreen(state, onBack, onPermissionsGranted, onPermissionsDenied, onScanClick, onSavedDeviceClick, onRemoveDeviceClick, onAddDeviceClick)` — Scaffold-обвязка вокруг `SensorsScreenContent`.
- `@Composable fun SensorsScreenContent(state, onPermissionsGranted, onPermissionsDenied, onScanClick, onSavedDeviceClick, onRemoveDeviceClick, onAddDeviceClick, modifier)` — минималистичный список: `BluetoothPermissionHandler`; секция сохранённых (`SavedDeviceRow` — точка статуса зелёная/янтарная/серая + имя + BPM у подключённого активного + корзина `ic_delete`; тап по строке = переключение) или подсказка; компактная pill-кнопка «Поиск» по центру (`ScanPillButton`); после скана — секция «Найденные» (`FoundDeviceRow` — имя + rssi + «+» `Icons.Filled.Add`) с `hasScanned`-гейтом и подсказкой при пустом результате. Хосты: `SensorsScreen` (маршрут) и `SensorsDialog` (диалог с экрана тренировки); размер задаёт хост через `modifier`, скролл внутри.
