 ...
 *
 * **Контракт отказоустойчивости (обязателен для всех реализаций):**
 * ни один метод НЕ бросает исключений. Хранилище вызывается из OkHttp-интерцептора
 * и Authenticator'а — не-IOException на dispatcher-потоке OkHttp роняет процесс
 * (OkHttp 4 перебрасывает его после уведомления callback'а). Сбой хранилища
 * (повреждённые EncryptedSharedPreferences, глюк Android Keystore) выражается
 * возвратом null/emptyList/false и логом — приложение деградирует до
 * «пользователь не авторизован» вместо краша.
 */
